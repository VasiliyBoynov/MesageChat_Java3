        ...
        while (running) {
            try {
                ...
                if (message.equals("/end")) {
                    Server.broadCastMessage(String.format("/removeClient Чат покинул %s%n",getNickName()));
                    DB.updateConnection(getNickName(),false);
                    output.writeUTF(message);
                    output.flush();
                    input.close();
                    output.close();
                ...
                else if (message.startsWith("/updateNick")){
                    Server.massageUpdateNick(message,getNickName(),this);
                }
                else  {
